...
export async function getTVDetails(req, res) {
    try {
    const {id} = req.params
    const details = await fetchFromTMDB(`https://api.themoviedb.org/3/tv/${id}?language=en-US`);
...
export async function getTVByCategory(req, res) {
    ...
    const data = await fetchFromTMDB(`https://api.thedb.org/3/TV/${category}?language=en-US&page=1`);
    res.status(200).json({success: true, s: data.results});
    } catch (error) {
        if(error.message.includes("404")) {
            return res.status(404).send(null)
        }
